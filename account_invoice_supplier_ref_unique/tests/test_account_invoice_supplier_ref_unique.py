...
class TestAccountInvoiceSupplierRefUnique(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        # Invoice with unique reference by year 'XYZ123'
        cls.account_invoice.create(
            {
                "partner_id": cls.partner.id,
                "account_id": cls.account.id,
                "type": "in_invoice",
                "supplier_invoice_number": "XYZ123",
                "date_invoice": "2024-04-20",
            }
        )
    ...
    def test_check_unique_supplier_invoice_number_insensitive_by_year(self):
        # A new invoice instance with an existing supplier_invoice_number
        with self.assertRaises(ValidationError):
            self.account_invoice.create(
                {
